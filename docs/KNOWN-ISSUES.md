...
## Hyper-V VMConnect: 最大サイズを保存して4Kモニターを占有する

### 症状

Enhanced Session の接続時に表示サイズを最大にし、`この仮想マシンへの今後の接続のために設定を保存する` を有効にしたところ、次回以降 `AI-Dev-Lab` が 4K モニター側を大きく占有して開くようになりました。

普段は FHD モニター側で VM を使っていたため、保存された display/session 設定が実運用と合わなくなり、通常のウィンドウサイズへ戻しにくい状態になりました。

### 原因

VM 本体や guest Windows の解像度設定が壊れたのではなく、**VMConnect に保存された接続セッション設定**が原因でした。
...
```powershell
vmconnect.exe localhost "AI-Dev-Lab" /edit
```

これで `AI-Dev-Lab` の保存済み VMConnect 接続設定を再編集できます。

表示サイズを普段使うモニターに合う値へ戻し、必要なら Remote Audio / local resources 等も確認してから再保存します。

### 補助ショートカット

VMConnect が full screen になっている場合は、次で full screen / windowed を切り替えられます。
...
- 最大サイズや特殊な monitor 構成を `設定を保存する` で固定する前に、普段使う monitor と解像度を確認する
- 4K / FHD の混在環境では、最大サイズ保存が意図しない monitor 占有につながることがある
- 表示設定がおかしくなっても、VM 自体を作り直さず `/edit` でまず接続設定を修正する
- VM をシャットダウンせず、VMConnect だけ閉じて設定を直せる
